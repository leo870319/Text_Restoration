...
async def main():
    ...
    if len(blocks) == 2:
        with open(f"{OUT_PATH}ToC.md", "w") as f: f.write(blocks[0])
        with open(f"{OUT_PATH}Glossary.md", "w") as f: f.write(blocks[1])
    else:
    ...
    for i, query_text in enumerate(queries, start=INDEX):
        ...
        md = doc.to_markdown()
        with open(f'{OUT_PATH}translated.md', 'w', encoding='utf-8') as f:
            f.write(md)
